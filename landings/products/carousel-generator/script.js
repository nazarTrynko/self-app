...
function init() {
  ...
  if (typeof CAROUSEL_TEMPLATES === "undefined") {
    console.error("CAROUSEL_TEMPLATES not loaded");
    return;
  }
  ...
  if (CAROUSEL_TEMPLATES && CAROUSEL_TEMPLATES.length > 0) {
...
function loadTemplates() {
  ...
  if (
    typeof CAROUSEL_TEMPLATES !== "undefined" &&
    Array.isArray(CAROUSEL_TEMPLATES)
  ) {
    ...
    templateList.innerHTML =
      '<p style="color: var(--text-muted); padding: 20px; text-align: center;">Templates loading...</p>';
  }
}
...
function setupEventListeners() {
  ...
  document
    .getElementById("duplicateSlideBtn")
...
function duplicateCurrentSlide() {
  ...
  currentTemplate.slides.splice(
    templateEngine.currentSlideIndex + 1,
    0,
    duplicatedSlide
  );
  templateEngine.loadTemplate(currentTemplate);
...
function deleteCurrentSlide() {
  ...
  if (confirm("Delete this slide?")) {
    ...
    const newIndex = Math.min(
      templateEngine.currentSlideIndex,
...
function updateEditor() {
    ...
    currentSlide.elements.forEach((element, index) => {
      ...
      item.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <span>${element.type} - ${
        element.content || element.shape || "Element"
      }</span>
                    <button class="btn-icon" style="width: 24px; height: 24px; font-size: 0.75rem;" data-element-index="${index}">×</button>
                </div>
            `;
      ...
      const deleteBtn = item.querySelector("button[data-element-index]");
      if (deleteBtn) {
        ...
        <div class="style-group">
            ...
            <select id="bgType">
                <option value="gradient" ${
                  currentSlide.background?.type === "gradient" ? "selected" : ""
                }>Gradient</option>
                <option value="solid" ${
                  currentSlide.background?.type === "solid" ? "selected" : ""
                }>Solid</option>
            </select>
        </div>
        <div class="style-group">
            <label>Background Color 1</label>
            <input type="color" id="bgColor1" value="${
              currentSlide.background?.colors?.[0] || "#08080c"
            }">
        </div>
        <div class="style-group">
            <label>Background Color 2</label>
            <input type="color" id="bgColor2" value="${
              currentSlide.background?.colors?.[1] || "#1a1a2e"
            }">
        </div>
    `;
  ...
  if (bgType) bgType.addEventListener("change", updateBackground);
  if (bgColor1) bgColor1.addEventListener("change", updateBackground);
  if (bgColor2) bgColor2.addEventListener("change", updateBackground);
}
...
function handleExport() {
  ...
  const dimensions =
    document.getElementById("exportDimensions")?.value || "1080x1080";
